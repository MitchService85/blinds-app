-- "Locked": the crew could not get into the unit. Independent of
-- install_blocked, which is our own factory/measure problem and stays
-- internal. Locked is the one exception the external PM view shows besides
-- done, because arranging access is theirs to do.
alter table units add column if not exists locked boolean not null default false;

-- pm_project_status gains 'locked' per unit (full body re-stated so the file
-- is the truth; identical to 004 otherwise).
